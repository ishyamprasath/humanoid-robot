// ============================================================
// Robot — configuration + persona
//
// Secrets (Gemini API key, model, voice) are NOT hardcoded here.
// They're injected at runtime by the server via /js/env.js which
// reads them from .env. See server.js + .env.example.
// ============================================================

const ENV = (typeof window !== "undefined" && window.__ROBOT_ENV__) || {};
if (!ENV.GEMINI_API_KEY) {
  console.warn("⚠️  GEMINI_API_KEY missing from /js/env.js — copy .env.example → .env and restart the server.");
}

export const CONFIG = {
  // --- Gemini Live API (primary brain), from server env ---
  GEMINI_API_KEY:     ENV.GEMINI_API_KEY     || "",
  GEMINI_MODEL:       ENV.GEMINI_MODEL       || "models/gemini-3.1-flash-live-preview",
  GEMINI_API_VERSION: ENV.GEMINI_API_VERSION || "v1beta",
  VOICE_NAME:         ENV.VOICE_NAME         || "Kore",
  GEMINI_WS_URL_TEMPLATE:
    "wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.{ver}.GenerativeService.BidiGenerateContent",

  // --- NVIDIA Nemotron (text fallback) ---
  NVIDIA_PROXY_ENDPOINT: "/api/nvidia",

  // --- Audio ---
  SEND_SAMPLE_RATE: 16000,
  RECV_SAMPLE_RATE: 24000,
  MIC_CHUNK_SAMPLES: 2048,
  ...
  // --- World frame (physical room, meters) ---
  // Room is 4 m x 4 m, origin (0,0) at center, +x = east, +y = north.
  ROOM_HALF_METERS: 2.0,
};

// ============================================================
// System prompt — the robot's mind
// ============================================================
...
- Arm reach maxes out at 1.5 m depth. Base can move and turn.
...
2. WORLD FRAME (physical room, meters): origin (0, 0) at room center, +x = east (right on map), +y = north (up on map). The room is 4 m × 4 m so valid positions are roughly x ∈ [−2, 2], y ∈ [−2, 2].

For look_at / grasp / camera-relative actions, use the CAMERA FRAME (x, y ∈ [0,1], z in meters).
...
### COGNITIVE RULES
1. NATIVE VISUAL GROUNDING: When someone says "look at the red cup", find it in your video, compute camera-frame (x, y) + depth (z), then call execute_robot_action("look_at", …). Never invent coordinates for something you can't see.
2. THINK IN TASKS, NOT GESTURES: When asked to DO something ("bring me my keys", "check the kitchen", "come here"), open an execute_task call describing the goal + target. Then chain motor calls (navigate_to, execute_robot_action(grasp), etc.) to fulfill it. Every task should have a clear finish state.
3. ACT FAST: One short sentence + tool call beats a long explanation.
4. SPATIAL LIMITS: Don't grasp anything with z > 1.5 m — move closer first with navigate_to or move_robot, then grasp.
